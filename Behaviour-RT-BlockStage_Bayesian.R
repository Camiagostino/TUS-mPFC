rt_tus_bayes_all_trials <- function(
  data_path,
  output_path,
  iter,
  cores,
...
df_tus_eeg <- df_tus_eeg %>%
  mutate(
    ...
    Error       = ifelse(df_tus_eeg$Error == 0,-0.5,0.5),
...
prior(normal(-0.0015,  0.2),  class = "b", coef = "PrevError"),
  prior(normal(0.0069, 0.2), class = "b", coef = "BlockStageMiddle"),
  prior(normal(0.0069, 0.2), class = "b", coef = "BlockStageEarly"),
  ...
  prior(normal(0, 0.2), class = "b", coef = "Stimulation1:BlockStageMiddle"),
  prior(normal(0, 0.2), class = "b", coef = "Stimulation3:BlockStageMiddle"),
  prior(normal(0, 0.2), class = "b", coef = "Stimulation1:BlockStageEarly"),
  prior(normal(0, 0.2), class = "b", coef = "Stimulation3:BlockStageEarly")
)
# -------------------- Model --------------------
model <- brm(
  RT ~ Stimulation*Congruence + Stimulation*Error + Stimulation*PrevError + Stimulation*BlockStage +
    (1 + Stimulation*Congruence + Stimulation*Error + Stimulation*PrevError + Stimulation*BlockStage || Subject),
  data   = df_tus_eeg,
  family = lognormal(),
