function varargout=AGF_shadedErrorBar(x,y,errBar,lineProps,transparent,varargin)
...
%               image. 
%OPTIONAL
%'modus'    - 'errbar' / 'ci' adds error bars or directly uses the value
...
if strcmp(Fmodus, 'errbar')    
    %Calculate the error bars
    uE=y+errBar(1,:);
    lE=y-errBar(2,:);
elseif strcmp(Fmodus, 'ci')
    uE=errBar(1,:);
    lE=errBar(2,:);
else
    disp('Unknown modus set.');return;
end;
