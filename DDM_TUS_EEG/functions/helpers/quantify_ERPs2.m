function [all_amps, all_times, all_results] = quantify_ERPs(inname, inlocation, MinMax, zeit, electrode, varargin)
...
%electrode: Number of Electrode to calculate with
%
%Optional: 
...
if strcmp(MinMax,'min')
    [amp1,time1]=min(EEG.data(electrode,zeit,:));display('*Taking Minimum in first timewindo')
elseif strcmp(MinMax,'max')
    [amp1,time1]=max(EEG.data(electrode,zeit,:));display('*Taking Maximum in first timewindo')
elseif strcmp(MinMax,'mean')
    amp1=mean(EEG.data(electrode,zeit,:));display('*Taking mean in first timewindo') 
    time1=NaN; 
end;
