%%%ENTER HERE CRITERIA
function [  ] = AGF_2nd_Level_Regression(infold, SLevelT, settings)
%%
...
if ~isempty(IncElect)
    IncElect = AGF_structfind(ChanLoc,'labels',IncElect);
else
    IncElect = AGF_structfind(ChanLoc, 'labels', {chanlocs(Elocs).labels});    
end;
...
if isstruct(TF)
    ...
    for c = 1:length(UseReg)
        ...
        %Run Regression
        [ Odata, add_info ] = AGF_Fast_Regress2(Reg2, [1:length(IncElect)], [Reg2.times(1) Reg2.times(end)], 'bin_size', 0, ...
             'stepsize',1, 'TF', [TF], 'model 1', {{1:length(VPN)} SLevelT {'EEG'} {'Level2Reg1'}},'interact',0,'binEEG', 3, 'retall', 3, ...
...
%     parfor (c = 1:length(UseReg), parforArg)
    for c = 1:length(UseReg)
        ...
        %Run Regression
        [ Odata, add_info ] = AGF_Fast_Regress2(Reg2, [1:length(IncElect)], [Reg2.times(1) Reg2.times(end)], 'bin_size', 0, ...
             'stepsize',1, 'TF', [], 'model 1', {{1:length(VPN)} SLevelT {'EEG'} {'Level2Reg1'}},'interact',0,'binEEG', 3, 'retall', 3, ...
