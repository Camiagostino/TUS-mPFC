...
%
% Usage:
%  >> [h crit_p adj_p]=fdr_bh(pvals,q,report,method,uplim)
%
% Required Input:
...
% 6/9/2012-Changed by AGF to provide ability to
%          enter maximal upper value  for crit_p!

function [h crit_p adj_p]=fdr_bh(pvals,q,report,method,uplim)
...
if isempty(max_id),
    ...
    if uplim & uplim<crit_p %If upper limit for FDR is specified, crit_p is adjusted to it
        crit_p=uplim;
    end;
    h=pvals<=crit_p;
end
