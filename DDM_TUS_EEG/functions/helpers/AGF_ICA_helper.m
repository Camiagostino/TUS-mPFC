...
for c = 5:size(filez,1)
    EEG = pop_loadset( 'filename', filez(c).name);
    [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
    EEG = eeg_checkset( EEG );
    [T, com] = pop_selectcomps(EEG, [1:20] ); %
    pause
    ...
    EEG = pop_saveset(EEG, 'filename', [filez(c).name]); %EEG.setname(1:7) 'ICA Comp Removed'
    ALLEEG = pop_delset(ALLEEG, [1  2]);
    close all
    
    %Store removed components in a matfile
    %RemComp.(EEG.setname(1:5)) = CR;
    %save('Removed Components.mat', 'RemComp')
end;
