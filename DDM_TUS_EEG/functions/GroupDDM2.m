...
elseif ModelSelect == 2
    ...
    VP.HyperPriors  = [PR.v;        PR.sv; PR.a;  PR.sa;    PR.ter; PR.st; PR.k]; %;  0 2     ;    0 5 fixed borders for all parameters (these are evaluated before the model call, which speeds up MCMC considerably)   
    VP.ParamNames   = {'DriftRate'  'sv'  'a'     'StartVar' 'Ter'   'st'  'k'}; %   'st''Collapse'
    VP.Params       = [1             2     3      4           5       6     7]; % 6  this maps input to model order of factors 
...
elseif ModelSelect == 4
    ...
    VP.HyperPriors  = [PR.v;        PR.sv; PR.a;  PR.sa;    PR.ter; PR.st; PR.k; PR.f]; %;  0 2     ;    0 5 fixed borders for all parameters (these are evaluated before the model call, which speeds up MCMC considerably)   
    VP.ParamNames   = {'DriftRate'  'sv'  'a'     'StartVar' 'Ter'   'st'  'k'   'f'}; %   'st''Collapse'
...
if FitMethod == 1
    ...
    DEParams.maxtime  = 1800;  % in seconds
    DEParams.maxclock = [];
