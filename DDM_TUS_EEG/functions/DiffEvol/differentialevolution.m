...
timeOverFileName = sprintf('%s_timeover_%s.mat', objFctName, 'DefaultHorst');
if exist('./data', 'dir')
...
if sendEmail && ~isempty(sendMailPeriod)
  ...
  elseif mbtime - nextSendMailTime >= 0
    ...
    if isfield(optimInfo, 'title')
      subject = [subject, sprintf(' (%s, host %s)', optimInfo.title, 'DefaultHorst')]; %gethostname
    else
      subject = [subject, sprintf(' (host %s)', 'DefaultHorst')];
    end
...
if sendEmailThisTime
  ...
  if isfield(optimInfo, 'title')
    subject = [subject, sprintf(' (%s, host %s)', optimInfo.title, 'DefaultHorst')];
  else
    subject = [subject, sprintf(' (host %s)', 'DefaultHorst')];
  end
